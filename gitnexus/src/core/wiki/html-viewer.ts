...
function buildHTML(
  ...
  const labels = getViewerLabels(meta);
  ...
  const labelsJSON = escScript(JSON.stringify(labels));

  const parts: string[] = [];

  // ── Head ──
  parts.push('<!DOCTYPE html>');
  parts.push('<html lang="' + esc(labels.htmlLang) + '">');
  parts.push('<head>');
  parts.push('<meta charset="UTF-8">');
  parts.push('<meta name="viewport" content="width=device-width, initial-scale=1.0">');
  parts.push('<title>' + esc(projectName) + esc(labels.titleSuffix) + '</title>');
  ...
  parts.push('<body>');
  parts.push(
    '<button class="menu-toggle" id="menu-toggle" aria-label="' +
      esc(labels.toggleMenu) +
      '">&#9776;</button>',
  );
  ...
  parts.push('<div class="sidebar-footer">' + esc(labels.generatedBy) + '</div>');
  parts.push('</nav>');

  // Content
  parts.push('<main class="content" id="content">');
  parts.push('<div class="empty-state"><h2>' + esc(labels.loading) + '</h2></div>');
  parts.push('</main>');
  parts.push('</div>');
  ...
  parts.push('var LABELS = ' + labelsJSON + ';');
  parts.push(JS_APP);
  parts.push('<\/script>');
...
function getViewerLabels(meta: Record<string, unknown> | null): ViewerLabels {
  if (meta?.lang === 'zh-CN') {
    return {
      htmlLang: 'zh-CN',
      titleSuffix: ' — Wiki',
      ...
      modules: '模块',
      pageNotFound: '页面不存在',
      pageMissing: '对应的 Markdown 页面不存在。',
    };
  }
  ...
  return {
    ...
    pageMissing: 'Markdown page does not exist.',
  };
}
...
(function() {
  ...
  function renderNav() {
    ...
    html += '<a class="nav-item overview" data-page="overview" href="#overview">' + escH(LABELS.overview) + '</a>';
    html += '</div>';
    if (TREE.length > 0) {
      html += '<div class="nav-group-label">' + escH(LABELS.modules) + '</div>';
  ...
  function navigateTo(page) {
    ...
    if (!md) {
      contentEl.innerHTML = '<div class="empty-state"><h2>' + escH(LABELS.pageNotFound) + '</h2><p>' + escH(page) + '.md ' + escH(LABELS.pageMissing) + '</p></div>';
      return;
    }
